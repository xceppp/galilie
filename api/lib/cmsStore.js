 ...
 * Singletons: cms_content | Lists: announcements, trust, formations, faq, cases, clients
 */
...
const HEADERS = {
  ...
  [TAB_ANNOUNCEMENTS]: ['id', 'text', 'active', 'order'],
  [TAB_TRUST]: ['id', 'bold', 'text', 'active', 'order'],
  [TAB_FORMATIONS]: [
    'id', 'tag', 'title', 'subtitle', 'items', 'cta_url', 'active', 'order',
  ],
...
function shapeAnnouncements(objs) {
  return shapeSimple(objs, ['id', 'text', 'active', 'order']);
}
  ...
  switch (tabName) {
    ...
    case TAB_ANNOUNCEMENTS:
      return DEFAULTS.announcements.map((a) => [
        a.id, a.text, a.active ? 'true' : 'false', a.order,
      ]);
    case TAB_TRUST:
...
async function readCms() {
  ...
  const content = shapeTab(TAB_CONTENT, rowSets[0]);
  const announcements = shapeTab(TAB_ANNOUNCEMENTS, rowSets[1]);
  const trust = shapeTab(TAB_TRUST, rowSets[2]);
  const formations = shapeTab(TAB_FORMATIONS, rowSets[3]);
  const faq = shapeTab(TAB_FAQ, rowSets[4]);
  const cases = shapeTab(TAB_CASES, rowSets[5]);
  const clients = shapeTab(TAB_CLIENTS, rowSets[6]);
...
async function writeCms(payload) {
  ...
  if (Array.isArray(payload.announcements)) {
    const rows = payload.announcements
      .filter((a) => a && (String(a.text || '').trim() || String(a.id || '').trim()))
      .map((a, i) => [
