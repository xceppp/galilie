...
/** True when all env vars required for admin sign-in are present. */
function isAdminAuthConfigured() {
  const clientId = String(process.env.GOOGLE_OAUTH_CLIENT_ID || '').trim();
  const secret = String(process.env.SESSION_SECRET || '').trim();
  return Boolean(clientId) && secret.length >= 16 && getAdminEmails().length > 0;
}

/** Lists missing env var names (for setup UI / diagnostics). */
function getAdminAuthSetupStatus() {
  const missing = [];
  if (!String(process.env.GOOGLE_OAUTH_CLIENT_ID || '').trim()) {
    missing.push('GOOGLE_OAUTH_CLIENT_ID');
  }
  if (!getAdminEmails().length) missing.push('ADMIN_EMAILS');
  const secret = String(process.env.SESSION_SECRET || '').trim();
  if (!secret || secret.length < 16) missing.push('SESSION_SECRET');
